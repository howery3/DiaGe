...
function toDateInputValue(isoFull: string): string {
  if (!isoFull) return "";
  return isoFull.slice(0, 10);
}
...
export default function EditReminderScreen() {
  ...
  const { reminders, pieces, updateReminder } = useDiGe();

  const reminder = reminders.find((r) => r.id === id);

  const [jewelryName, setJewelryName] = useState(reminder?.jewelryName ?? "");
  const [retailer, setRetailer] = useState(reminder?.retailer ?? "");
  const [scheduledDate, setScheduledDate] = useState(
    reminder ? toDateInputValue(reminder.scheduledDate) : ""
  );
  const [recurrence, setRecurrence] = useState<ReminderRecurrence>(
    reminder?.recurrence ?? "1year"
  );
      ...
      >
        ...
        {pieces.length > 0 ? (
          <View style={styles.field}>
            ...
            {showPieces ? (
              <View style={[styles.pieceList, { backgroundColor: colors.card, borderColor: colors.border }]}>
                {pieces.map((p) => (
                  <Pressable
                    key={p.id}
          ...
          {RECURRENCES.map((r) => (
            <Pressable
              ...
              style={[
                styles.recPill,
                {
                  backgroundColor: recurrence === r.value ? colors.primary + "15" : colors.secondary,
                  borderColor: recurrence === r.value ? colors.primary : colors.border,
                  borderWidth: 1.5,
                },
              ]}
            >
              <Text style={[styles.recLabel, { color: recurrence === r.value ? colors.primary : colors.foreground }]}>
                {r.label}
              </Text>
...
const styles = StyleSheet.create({
  ...
  selectBtn: { borderWidth: 1, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 11 },
  selectBtnText: { fontSize: 14, fontFamily: "Inter_400Regular" },
  pieceList: { borderWidth: 1, borderRadius: 10, marginTop: 4, overflow: "hidden" },
  pieceItem: { padding: 12, borderBottomWidth: StyleSheet.hairlineWidth },
  ...
  recPill: { width: "47%", padding: 12, borderRadius: 12, alignItems: "center", gap: 2 },
